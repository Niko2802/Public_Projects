...
class Level():
    def __init__(self, level_number=1) -> None:
        level = f"level_{level_number}"
        with open("Labber_3/level.json", "r") as f:
            level_data = json.load(f)[level]
        self.items = []
        self.hero=eo.Hero(50, 50)
        walls=eo.Wall.create(*level_data["walls"])
        ...
        #                                                                                                                                                                                       500),                                                                                                                                                                       (750, 500),
        # )
        coins=eo.Coin.create(*level_data["coins"])
        ...
        gates=eo.Gate.create(*level_data["gates"])
        chests=eo.Chest.create(*level_data["chests"])
        sg=eo.SwitchedGate(400, 200)
        layer=eo.Layer(50, 150, sg)
